...
<tr>
	...
	<tr>
	<td>Nilai Matematika</td><td><input type="text" name="e" id="e" autocomplete="off" required="" minlength="1"></td></tr>
<tr>
<td>Nilai Pkn</td>
...
if (isset($_POST['kirim'])) {
	...
	$nilai_matematika=htmlspecialchars($_POST['e']);
	$nilai_pkn=htmlspecialchars($_POST['f']);
	
	$koneksi= new mysqli("localhost", "root", "", "mahasiswa");/* baris ini untuk menghubungkan kedatabase */
	$sql="INSERT INTO `datamahasiswa` VALUES ('".$npm."', '".$nama."', '".$nilai_bahasa."', '".$nilai_agama."', '".$nilai_matematika."', '".$nilai_pkn."');";
	$q=$koneksi->query($sql);
if ($q) {
	echo "<script> alert(' data BERHASIL tambahkan');
document.location.href = 'daftar mahas(iswa.php';
